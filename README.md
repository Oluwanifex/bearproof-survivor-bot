...
`DESIRED_SCORE` is used only by the ordinary benchmark. `npm run today` fetches today's live contract, requires Build 4, refuses known physics/tick-cap overrides, runs twice, and checks that the replay hash and score match; its default threshold is 250,000 and it refuses to mark an early-death run as a submission candidate. `npm run tune:build4` runs the deterministic per-seed strategy search and replay-checks its candidates. A submission is only valid when the replay matches the exact Bearproof build, seed, twist, character, and simulation version.

On 2026-09-26, the active Build #4 board was **Crypto Winter / Flash Crash**, seed `526031759`. The best strategy candidate observed in this update scored **134,020** with **4,935 kills**, three bosses defeated, and a replay that verifies against the local Build 4 simulator—but it was liquidated after 41,030 ticks, so it is **not** a valid competition submission and does not meet the requested 250,000. The 250,000 target remains unachieved. See [Build 4 validation notes](docs/build4-results.md), [source provenance](docs/build4-sources.md), and the [current policy/deployment guardrails](docs/railway-variables.md). Build 3's 313,836 score is historical only and is not evidence for Build 4.
...
The Build 4 controller selects a character-legal daily policy by default. Do not deploy a profile as a competition solution based only on a local score: verify a full-cap/won replay and the site's exact submission contract first. Railway runtime variables belong in the Variables tab, not `railway.json`; see [current Build 4 settings and guardrails](docs/railway-variables.md).
