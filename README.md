...
npm run today
npm run tune:build4
SEARCH_COUNT=2000 npm run search:build4
```

`DESIRED_SCORE` is used only by the ordinary benchmark. `npm run today` fetches today's live contract, requires Build 4, refuses known physics/tick-cap overrides, runs twice, and checks replay hash/score determinism. Its default target is **322,000**; a lower result is reported honestly without treating a legal in-cap death as a simulation-rule violation. `npm run tune:build4` runs the deterministic per-seed strategy tuner and replay-checks its candidates. `SEARCH_COUNT=2000 npm run search:build4` samples controller and legal upgrade-policy variants against the live daily seed, reports score/XP/boss metrics, and replay-verifies its top five. It refuses known physics overrides and does not submit scores. Local replay compliance is not a claim of server acceptance; any submission still needs the exact Bearproof build, seed, twist, character, and simulation version.

For local refinement, `SEARCH_SEEDS_FILE=/path/to/results.jsonl SEARCH_COUNT=1000 npm run search:build4` mutates the leading configurations from a prior search. `SEARCH_DECOY_ONLY=1` forces sampled candidates to activate the configurable XP decoy more often, `SEARCH_EARLY_FINAL_BOSS=1` biases attacks to start after the official final-boss spawn, and `SEARCH_FINAL_BOSS_GRID=1` adds a fixed timing grid around top seed policies. Add `SEARCH_GRID_UPGRADES=1` to that grid to compare all legal upgrade scorers at each timing. These switches affect search sampling only. The production runner keeps its current upgrade selection by default; `BUILD4_UPGRADE_PROFILE` can explicitly select one of the searched legal profiles (`combat`, `offense`, `defense`, `xp`, `leveraged`, `glass`) when reproducing a tested candidate.

On 2026-09-26, the active Build #4 board was **Crypto Winter / Flash Crash**, seed `526031759`. A search of thousands of legal controller/upgrade configurations found a **331,118-point four-boss win** at 70,708 ticks; the replay was independently verified against hash `135ecf39`. The date-specific preset applies only when the live challenge matches that exact date, build, stage, seed, and twist; set `BUILD4_TARGET_PRESET=0` to disable it. For the winning config, the decoy was activated 28 times, returned 27 times, and recovered 21 XP orbs on return (4 target orbs expired). The official 72,000-tick cap and all game physics remain unchanged. See the [331,118 result and winning settings](docs/build4-331118-result.md), [Build 4 validation notes](docs/build4-results.md), [source provenance](docs/build4-sources.md), and [policy/deployment guardrails](docs/railway-variables.md). Build 3's 313,836 score is historical only and is not evidence for Build 4.
