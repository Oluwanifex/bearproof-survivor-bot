...
const rows = [];
for (const level of [31, 32, 33]) for (const index of [0, 1, 2]) {
  ...
  const row = { level, index, score: r.score, timeMs: r.timeMs, kills: r.kills, bossKills: r.bossKills, reason: r.reason, finalBossDamage: r.telemetry?.bossDamageById?.bear_market || 0, hash: r.hash };
  rows.push(row);
  console.error(JSON.stringify(row));
}
rows.sort((a, b) => b.score - a.score);
console.log(JSON.stringify({ best: rows, all: rows }, null, 2));
