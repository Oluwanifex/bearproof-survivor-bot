...
const rows = [];
for (const magnet of [1.1, 1.15, 1.2, 1.25, 1.3, 1.35, 1.4]) for (const target of [240, 270, 300, 330, 360]) for (const crowd of [0, 1]) for (const attraction of [0.01, 0.011, 0.012, 0.013, 0.014]) {
  ...
  const row = { magnet, target, crowd, attraction, score: r.score, timeMs: r.timeMs, kills: r.kills, bossKills: r.bossKills, won: r.won, reason: r.reason, xpCollected: r.telemetry?.xpCollected, xpExpired: r.telemetry?.xpExpired, hash: r.hash };
  rows.push(row);
